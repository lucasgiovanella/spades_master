...
"""
JobManager - Gerencia execuções remotas do SPAdes via SSH
Parte 1: Conexão e gerenciamento básico
"""
...
from backend.utils.logging_utils import log_info, log_error, log_warning, log_success
...
class JobManager:
    ...
    def __init__(self, status_updater):
        ...
        Args:
            status_updater: Instância do SocketUpdater para comunicação em tempo real
        """
    ...
    def connect(self, host, port, username, password=None, key_path=None, use_key=False):
        ...
        try:
            ...
            if spades_path == 'NOT_FOUND':
                self.status_updater.update_status("Aviso: SPAdes não encontrado automaticamente")
                self.status_updater.update_log("SPAdes não encontrado automaticamente no servidor. Você precisará especificar o caminho manualmente.", "WARNING")
                spades_path = None
    ...
    def disconnect(self):
        ...
        # Primeiro parar qualquer monitoramento em andamento
    ...
    def check_server_resources(self):
        ...
        try:
            ...
            if isinstance(cpu_count, int) and cpu_count < 2:
                cpu_ok = False
                self.status_updater.update_log(f"Atenção: Apenas {cpu_count} CPU(s) disponível(is). Recomendado: 2+", "WARNING")
                
            if isinstance(total_mem, int) and total_mem < 4000:
                mem_ok = False
                self.status_updater.update_log(f"Atenção: Apenas {total_mem} MB de RAM. Recomendado: 4+ GB", "WARNING")
                
            if disk_avail != "unknown" and not disk_avail.endswith("G") and not disk_avail.endswith("T"):
                disk_ok = False
                self.status_updater.update_log(f"Atenção: Pouco espaço em disco disponível: {disk_avail}. Recomendado: 10+ GB", "WARNING")
            ...
            self.status_updater.update_log(f"Recursos do servidor: {cpu_count} CPUs, {total_mem} MB RAM, {disk_avail} disco disponível", "INFO")
    ...
    def run_spades(self, remote_dir, read1, read2, output_dir, threads, memory=None, mode="isolate", kmer=None, advanced_params=None):
        ...
        Args:
            ...
            advanced_params: Dicionário com parâmetros avançados (opcional)
        ...
        try:
            ...
            # Adicionar parâmetros avançados, se fornecidos
            if advanced_params:
                # Cutoff de cobertura
                if 'cov_cutoff' in advanced_params:
                    command += f" --cov-cutoff {advanced_params['cov_cutoff']}"
                
                # Offset de qualidade Phred
                if 'phred_offset' in advanced_params:
                    command += f" --phred-offset {advanced_params['phred_offset']}"
                
                # Opções de pipeline
                if advanced_params.get('only_error_correction', False):
                    command += " --only-error-correction"
                
                if advanced_params.get('only_assembler', False):
            ...
            # Iniciar thread para monitorar o processo - modificado para websockets
        ...
        def _monitor_job(self, remote_dir, pid, job_id, output_dir):
            ...
            Args:
                ...
                output_dir: Diretório de saída
            """
            self.status_updater.update_log(f"Iniciando monitoramento do job {job_id} (PID: {pid})")
        try:
            ...
            while self.job_running and self.connected and not self.monitor_stop_event.is_set():
                try:
                    ...
                    if process_info:
                        ...
                        try:
                            ...
                            if len(parts) >= 3:
                                metrics['cpu_usage'] = float(parts[1])
                                metrics['memory_usage'] = float(parts[2])
                                metrics['runtime'] = parts[3] if len(parts) > 3 else "00:00:00"
                        except:
                            pass
                        
                        # Enviar atualização para cliente
                        if metrics:
                            self.status_updater.update_metrics(metrics)
                        
                        # Verificar log do SPAdes para extrair a fase atual
                        ...
                        # Registrar saídas relevantes no log
                        if log_output or err_output:
                            # Extrair informações de progresso, se disponíveis
                            if log_output:
                                progress_info = self._parse_spades_progress(log_output)
                                if progress_info:
                                    self.status_updater.update_status(f"SPAdes executando - {progress_info}")
                                    
                        # Também verificar diretamente o diretório de saída para sinais de progresso
                        stdin, stdout, stderr = self.ssh.exec_command(f"find {remote_dir}/{output_dir} -type f -name '*.fasta' 2>/dev/null | wc -l")
                        fasta_count = stdout.read().decode().strip()
                        
                        if fasta_count and fasta_count.isdigit() and int(fasta_count) > 0:
                            # Há arquivos fasta - progresso significativo
                            self.status_updater.update_log(f"Arquivos FASTA encontrados no diretório de saída: {fasta_count}", "INFO")
                    else:
                        # Processo não encontrado ou terminado
                        no_response_count += 1
                        
                        # Verificar se realmente terminou ou se há algum problema
                        if no_response_count >= max_no_response:
                            # Várias tentativas sem resposta, considerar processo terminado
                            # Verificar código de saída, se possível
                            exit_status_found = False
                            
                            # Verificar se há arquivos de saída que indicam conclusão
                            stdin, stdout, stderr = self.ssh.exec_command(f"[ -f {remote_dir}/{output_dir}/scaffolds.fasta ] && echo 'SUCCESS' || echo 'NOT_FOUND'")
                            has_scaffolds = stdout.read().decode().strip() == 'SUCCESS'
                            
                            if has_scaffolds:
                                self.status_updater.update_log("Montagem concluída com sucesso! O arquivo scaffolds.fasta foi gerado.", "SUCCESS")
                                self.status_updater.update_status("SPAdes concluído com sucesso")
                                exit_status_found = True
                                success = True
                            else:
                                # Verificar contigs, caso o scaffolding tenha sido desativado
                                stdin, stdout, stderr = self.ssh.exec_command(f"[ -f {remote_dir}/{output_dir}/contigs.fasta ] && echo 'SUCCESS' || echo 'NOT_FOUND'")
                                has_contigs = stdout.read().decode().strip() == 'SUCCESS'
                                
                                if has_contigs:
                                    self.status_updater.update_log("Montagem concluída com sucesso! O arquivo contigs.fasta foi gerado.", "SUCCESS")
                                    self.status_updater.update_status("SPAdes concluído com sucesso")
                                    exit_status_found = True
                                    success = True
                                else:
                                    # Verificar se há logs de erro
                                    stdin, stdout, stderr = self.ssh.exec_command(f"grep -i error {remote_dir}/spades_{job_id}.log | tail -10")
                                    error_log = stdout.read().decode().strip()
                                    
                                    if error_log:
                                        self.status_updater.update_log(f"Erros encontrados no log: {error_log}", "ERROR")
                                        self.status_updater.update_status("SPAdes falhou com erros")
                                        exit_status_found = True
                                        success = False
                            
                            if not exit_status_found:
                                self.status_updater.update_log("O processo SPAdes terminou, mas não foi possível determinar se houve sucesso.", "WARNING")
                                self.status_updater.update_status("SPAdes terminou (status desconhecido)")
                                success = False
                            ...
                            self.status_updater.update_job_status({
                                'status': 'completed',
                                'success': success,
                                'remote_dir': remote_dir,
                                'output_dir': output_dir,
                                'message': 'Montagem concluída com sucesso!' if success else 'Montagem falhou ou foi cancelada'
                            })
                    ...
                    if no_response_count >= max_no_response:
                        ...
                        self.status_updater.update_job_status({
                            'status': 'error',
                            'success': False,
                            'message': 'Falha ao monitorar o processo SPAdes'
                        })
                        
                        break
                
                # Usar pausa com evento para permitir interrupção graciosa
                self.monitor_stop_event.wait(timeout=5.0)
    ...
    def check_job_status(self):
        ...
        """
        if not self.connected or not self.ssh or not self.job_pid:
            return False
            
        try:
            stdin, stdout, stderr = self.ssh.exec_command(f"ps -p {self.job_pid} -o pid= 2>/dev/null")
    ...
    def cancel_job(self):
        ...
        try:
            ...
            # Parar o monitoramento se estiver rodando
            if self.monitoring_thread and self.monitoring_thread.is_alive():
                self.monitor_stop_event.set()
                self.monitoring_thread.join(timeout=2.0)
                self.monitoring_thread = None
            ...
            if output_dir:
                ...
                try:
                    ...
                    if remaining > 0:
                        ...
                        return True, f"Limpeza parcial: {file_count + dir_count - remaining} itens removidos, {remaining} itens restantes"
                    else:
                        return True, f"Limpeza completa: {file_count} arquivos e {dir_count} diretórios removidos"
